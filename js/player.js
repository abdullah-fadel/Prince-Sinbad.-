...
function updatePlayer(dt){
  ...
  if (fp && !fHeld && P.cool <= 0 && P.fire > 0 && P.rollT <= 0 && P.swordT <= 0){
    ...
    /* spawn from the hero's own hand — chest height if real raised-arm cast
       frames are active, else his actual (lowered) hand in the static pose */
    const fy = (typeof heroFireReady === 'function' && heroFireReady()) ? P.y - 4 : P.y + 10;
    G.fireballs.push({ x:P.x + P.w / 2 + P.face * 30, y:fy, vx:P.face * 520, vy:0, t:0, r:11 });
